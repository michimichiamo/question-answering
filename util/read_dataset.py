...
def read_csv(path='./', split=None):
	assert split in ['train', 'val']
	filename = path+'data/tokenized/'+split+'.npz'
	data = np.load(filename)

	x0 = data['input_ids'].astype('int32')
	x1 = data['attention_mask'].astype('int32')
	y0 = data['answer_start'].astype('int32')
	y1 = data['answer_end'].astype('int32')
